...
module.exports = function generateSchema (spaceschema, { esm = false, filename } = {}) {
  const structs = []
  ...
  for (let i = 0; i < spaceschema.schema.length; i++) {
    const fqn = spaceschema.typesByPosition.get(i)
    const type = spaceschema.resolve(fqn)
    if (type.isStruct) {
      structs.push(type)
    }
    if (type.isArray) {
  ...
  str += '// This file is autogenerated by the spaceschema compiler\n'
  str += `// Schema Version: ${spaceschema.version}\n`
  str += '/* eslint-disable camelcase */\n'
  str += '/* eslint-disable quotes */\n'
  str += '\n'
  str += `const VERSION = ${spaceschema.version}\n`

  if (esm) {
    str += 'import { c } from \'spaceschema/runtime\'\n'
  } else {
    str += 'const { c } = require(\'spaceschema/runtime\')\n'
  }
